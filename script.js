...
.task-actions {
    ...
    right: 10px;
    top: 50%;
    transform: translateY(-50%); /* Mantenemos el centrado vertical */
    display: flex;
    background: rgb(0,0,0,0);
    border-radius: 20px;
    opacity: 0;
    pointer-events: none;
    transition: opacity 0.15s ease-in; /* Hacemos que el contenedor se desvanezca un poco más rápido */
    z-index: 5;
    padding: 1px;
}
.task-item.selected .task-actions {
    opacity: 1;
    pointer-events: auto;
    transform: translateY(-50%); /* El contenedor ya no se desliza */
}
