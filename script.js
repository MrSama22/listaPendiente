...
const firebaseConfig = {
  ...
  auth.onAuthStateChanged(user => {
      if (user) {
          ...
          initTaskListeners(user.uid); // Carga/sincroniza tareas
      } else {
          appBox.style.display = 'none';
          authBox.style.display = 'block';
          unsubscribe && unsubscribe(); // Detiene listener a Firestore si existía
      }
  });
  ...
  // ---------- 3. Firestore ----------
  let tasks = []; // cache en memoria
  let tasksCol, unsubscribe; // listener en tiempo real
